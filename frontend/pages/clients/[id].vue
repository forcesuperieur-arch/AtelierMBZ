  ...
  <div>
    <div class="page-header">
      <div style="display:flex;align-items:center;gap:12px;">
        <NuxtLink to="/clients" style="color:var(--content-3);text-decoration:none;font-size:18px;">◀</NuxtLink>
      ...
      <div class="detail-main">
        ...
        <UCard>
          ...
          <div v-if="client.notes" style="margin-top:12px;padding:10px;background:var(--accent-soft);border:1px solid var(--accent);border-radius:8px;font-size:13px;">
            <span style="color:var(--accent-content);font-weight:600;">📌 Notes :</span>
        ...
        <UCard>
          <template #header>
            <span style="font-size:15px;font-weight:700;color:var(--content-1);">🏍 Carnet Moto</span>
          </template>
          <div v-if="client.vehicules?.length" style="display:flex;flex-direction:column;gap:16px;">
            <div v-for="v in client.vehicules" :key="v.id" style="border:1px solid var(--border-2);border-radius:12px;overflow:hidden;">
              ...
              <div @click="toggleVehicle(v.id)" style="display:flex;align-items:center;justify-content:space-between;padding:14px;cursor:pointer;background:var(--overlay-soft);transition:background 0.15s;" class="hover-row">
                <div style="display:flex;align-items:center;gap:12px;">
                  <div style="width:36px;height:36px;border-radius:10px;background:var(--accent-soft);display:flex;align-items:center;justify-content:center;font-size:16px;">🏍</div>
                  <div>
                ...
                <div style="display:flex;align-items:center;gap:8px;">
                  ...
                  <span style="font-size:14px;color:var(--content-3);transition:transform 0.2s;" :style="{ transform: expandedVehicles.includes(v.id) ? 'rotate(180deg)' : '' }">▼</span>
                </div>
              </div>
        ...
        <!-- RDV History -->
        <UCard>
          <template #header><span style="font-size:15px;font-weight:700;color:var(--content-1);">📅 Historique complet</span></template>
      ...
      <div class="detail-side">
        <UCard>
          ...
          <div style="display:flex;flex-direction:column;gap:14px;font-size:13px;">
            <div style="display:flex;justify-content:space-between;"><span style="color:var(--content-3);">📅 Visites</span><span style="font-weight:700;font-size:18px;color:var(--content-1);">{{ clientRdvs.length }}</span></div>
            <div style="display:flex;justify-content:space-between;"><span style="color:var(--content-3);">🏍 Motos</span><span style="font-weight:700;font-size:18px;color:var(--content-1);">{{ client.vehicules?.length || 0 }}</span></div>
            <div style="display:flex;justify-content:space-between;"><span style="color:var(--content-3);">💰 CA Total</span><span style="font-weight:700;font-size:18px;color:var(--accent-content);">{{ formatCurrency(caTotal) }}</span></div>
            <div style="display:flex;justify-content:space-between;"><span style="color:var(--content-3);">📆 Client depuis</span><span style="font-weight:600;color:var(--content-1);">{{ formatDate(client.created_at || client.createdAt) }}</span></div>
          </div>
        </UCard>
        ...
        <UCard v-if="!client.isAnonymized">
          <template #header><span style="font-size:15px;font-weight:700;color:var(--content-1);">🔒 RGPD</span></template>
        ...
        <UCard>
          <template #header>
            <div style="display:flex;justify-content:space-between;align-items:center;">
              ...
              <button @click="showEditClient = false" style="background:none;border:none;color:var(--content-3);font-size:18px;cursor:pointer;">✕</button>
            </div>
          </template>
