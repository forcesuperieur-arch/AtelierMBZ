...
from models import (
    ...
    Facture, LigneFacture, Paiement, UserAtelierRole, RolePermission
)
from auth import (
    verify_password, create_access_token, create_refresh_token, decode_token,
    revoke_token, get_current_user, get_current_atelier_id, get_password_hash, is_password_strong,
    create_default_users, ACCESS_TOKEN_EXPIRE_MINUTES, REFRESH_TOKEN_EXPIRE_DAYS
)
...
def login(response: Response, request: Request, form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    now = datetime.utcnow()
    attempts = [t for t in app.state.login_attempts.get(ip, []) if now - t < timedelta(minutes=1)]
    if len(attempts) >= 5:
    ...
    if not user or not verify_password(form_data.password, user.hashed_password):
        attempts.append(now)
        app.state.login_attempts[ip] = attempts
...
@app.post("/api/auth/switch-atelier")
def switch_atelier(
...
@app.post("/api/roles/permissions")
...
@app.delete("/api/roles/permissions/{role}")
...
class UserCreate(BaseModel):
    ...
    email: Optional[str] = None
    password: str
    role: str  # admin, receptionnaire, service_client, mecanicien
    atelier_id: Optional[int] = None
...
def _deactivate_mecanicien_for_user(db: Session, user: User):
    ...
    for m in mecanos:
        m.is_active = 0
        # Desaffecter uniquement les RDV non finalises/futurs pour garder l'historique passe
        db.query(RendezVous).filter(
            RendezVous.mecanicien_id == m.id,
            or_(
        ...
        db.query(Pont).filter(Pont.mecanicien_id == m.id).update({"mecanicien_id": None}, synchronize_session=False)
...
def invite_user_to_atelier(
    ...
    """Invitation simple: cree user si absent + assigne role atelier."""
...
@app.get("/api/ateliers/{atelier_id}/users", response_model=List[UserResponse])
@require_role("admin")
def get_atelier_users(
    ...
    for u in users:
        item = {
            ...
            "atelier_id": u.atelier_id
        }
        if u.role == "mecanicien":
            mec = db.query(Mecanicien).filter(
                Mecanicien.user_id == u.id,
...
def create_atelier(data: AtelierCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    db.commit()
    db.refresh(a)
    # Le super_admin createur est rattache automatiquement au nouvel atelier
    rel = db.query(UserAtelierRole).filter(
        UserAtelierRole.user_id == current_user.id,
        UserAtelierRole.atelier_id == a.id
    ).first()
    if not rel:
        db.add(UserAtelierRole(user_id=current_user.id, atelier_id=a.id, role="super_admin"))
        db.commit()
    return {"id": a.id, "nom": a.nom, "slug": a.slug}


@app.put("/api/ateliers/{atelier_id}")
def update_atelier(
    atelier_id: int,
    data: AtelierUpdate,
    db: Session = Depends(get_db),
    ...
    if current_user.role != "super_admin" and (current_user.atelier_id or 1) != atelier_id:
        raise HTTPException(status_code=403, detail="Acces atelier refuse")
    a = db.query(Atelier).filter(Atelier.id == atelier_id).first()
    if not a:
    ...
    db.commit()
    db.refresh(a)
    return {"id": a.id, "nom": a.nom, "slug": a.slug, "actif": a.actif}
...
def get_pieces(
    ...
    for piece in pieces:
        result.append({
            ...
            } if piece.fournisseur else None
        })
    
    return result


@app.get("/api/pieces/alertes")
...
def get_piece(
    ...
    return {
        ...
        "historique_commandes": historique_commandes[:10],  # 10 dernières commandes
        "utilisations_recentes": utilisations[:10]  # 10 dernières utilisations
    }
...
def delete_piece(
    ...
    """Supprime une pièce (soft delete)"""
    atelier_id = current_user.atelier_id or 1
    piece = db.query(PieceDetachee).filter(
        PieceDetachee.id == piece_id,
        PieceDetachee.atelier_id == atelier_id
...
@app.post("/api/pieces/{piece_id}/ajuster-stock")
def ajuster_stock(
    piece_id: int,
    quantite: int,
    ...
    """Ajuste le stock d'une pièce (ajout ou retrait)"""
    atelier_id = current_user.atelier_id or 1
    piece = db.query(PieceDetachee).filter(
        PieceDetachee.id == piece_id,
        PieceDetachee.atelier_id == atelier_id
