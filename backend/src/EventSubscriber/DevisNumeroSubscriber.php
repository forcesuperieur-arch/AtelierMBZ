...
class DevisNumeroSubscriber
{
    public function __construct(
        private DevisNumberingService $numberingService,
    ) {}
    ...
    public function prePersist(LifecycleEventArgs $args): void
    {
        ...
        // getNumeroDevis() est typé string non-nullable : Reflection pour tester l'initialisation.
        $ref = new \ReflectionProperty($entity, 'numeroDevis');
        if ($ref->isInitialized($entity) && $entity->getNumeroDevis() !== '') {
            return;
        }

        $entity->setNumeroDevis($this->numberingService->nextNumber());
    }
}
