<template>
  ...
  <div v-else>
    ...
    <div class="page-header" style="display:flex;align-items:center;gap:16px;flex-wrap:wrap;">
      <NuxtLink to="/devis" style="color:var(--content-3);text-decoration:none;font-size:20px;">←</NuxtLink>
    ...
    <div class="detail-summary-grid">
      <UCard>
        <template #header><span style="font-size:13px;font-weight:600;color:var(--content-3);">👤 Client</span></template>
      ...
      <UCard>
        <template #header><span style="font-size:13px;font-weight:600;color:var(--content-3);">🏍 Véhicule</span></template>
        <div style="font-size:14px;line-height:1.8;">
      ...
      <UCard>
        <template #header><span style="font-size:13px;font-weight:600;color:var(--content-3);">📋 Infos</span></template>
        <div style="font-size:14px;line-height:1.8;">
    ...
    <UCard style="margin-bottom:20px;">
      <template #header><span style="font-size:13px;font-weight:600;color:var(--content-3);">📝 Lignes</span></template>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:16px;margin-bottom:20px;">
      <UCard v-if="devis.notes_client || devis.notesClient">
        <template #header><span style="font-size:13px;font-weight:600;color:var(--content-3);">📌 Notes client</span></template>
      ...
      <UCard v-if="devis.notes_internes || devis.notesInternes">
        <template #header><span style="font-size:13px;font-weight:600;color:var(--content-3);">🔒 Notes internes</span></template>
    ...
    <!-- Actions -->
    <UCard>
      <template #header><span style="font-size:13px;font-weight:600;color:var(--content-3);">⚡ Actions</span></template>
      <div style="display:flex;flex-wrap:wrap;gap:10px;">
        ...
        <button v-if="['envoye','accepte'].includes(devis.statut) && client?.email" class="btn btn-ghost" @click="sendDevisEmail" :disabled="sendingEmail" style="color:var(--info-content);">{{ sendingEmail ? 'Envoi…' : '📧 Renvoyer par email' }}</button>
        ...
        <button v-if="['brouillon','envoye'].includes(devis.statut)" class="btn btn-ghost" @click="action('supprimer')" :disabled="acting" style="color:var(--error-content);">🗑 Supprimer</button>
      </div>
    </UCard>
  </div>
</template>
