    ...
    <div class="vo-wizard-grid">
      <div>
        <PitCard class="bg-white" v-if="step === 1">
          ...
          <UFormField label="Rechercher un client existant">
            <PitInput v-model="deposantSearch" placeholder="Nom, prénom, téléphone..." />
          </UFormField>
          ...
          <div class="vo-form-grid">
            ...
            <PitInput v-model="deposantForm.prenom" />
          </UFormField>
            <UFormField label="Nom">
            <PitInput v-model="deposantForm.nom" />
          </UFormField>
            <UFormField label="Téléphone">
            <PitInput v-model="deposantForm.telephone" />
          </UFormField>
            <UFormField label="Email">
            <PitInput v-model="deposantForm.email" />
          </UFormField>
            <UFormField label="Adresse" class="vo-field-full">
            <PitInput v-model="deposantForm.adresse" />
          </UFormField>
          </div>
        </PitCard>
        ...
        <PitCard class="bg-white" v-else-if="step === 2">
          ...
          <div class="vo-inline-search">
            <label class="vo-field grow">
              ...
              <PitInput v-model="vehicleSearch" placeholder="Ex: AB-123-CD ou VIN" />
            </label>
          ...
          <div class="vo-form-grid">
            <UFormField label="Immatriculation">
            <PitInput v-model="vehicleForm.plaque" />
          </UFormField>
            <label class="vo-field">
              <span>VIN</span>
              <PitInput v-model="vehicleForm.vin"  :class="{ 'vo-input-error': vehicleForm.vin && !isValidVin(vehicleForm.vin) }" />
              <span v-if="vehicleForm.vin && !isValidVin(vehicleForm.vin)" class="vo-field-error">{{ vinErrorMessage(vehicleForm.vin) }}</span>
            </label>
            <label class="vo-field">
              <span>Marque</span>
              <PitInput v-model="vehicleForm.marque"  @input="onVehicleMarqueInput" @blur="hideVehicleMarqueSuggestions" />
            ...
            <label class="vo-field">
              <span>Modèle</span>
              <PitInput v-model="vehicleForm.modele"  @input="onVehicleModeleInput" @blur="hideVehicleModeleSuggestions" />
            ...
            <UFormField label="Catégorie tarifaire">
            <PitSelect v-model="vehicleForm.categorieId" class="vo-select">
                <option value="">Non renseignée</option>
                <option v-for="category in categories" :key="category.id" :value="String(category.id)">{{ category.nom }}</option>
              </PitSelect>
          </UFormField>
            <UFormField label="Type véhicule atelier">
            <PitSelect v-model="vehicleForm.typeMoto" class="vo-select">
            ...
            <UFormField label="Cylindrée">
            <PitInput v-model="vehicleForm.cylindree" placeholder="ex: 750" />
          </UFormField>
            <UFormField label="Année">
            <PitInput v-model="vehicleForm.annee" type="number" />
          </UFormField>
            <UFormField label="Kilométrage">
            <PitInput v-model="vehicleForm.mileage" type="number" />
          </UFormField>
            <UFormField label="Couleur">
            <PitInput v-model="vehicleForm.couleur" />
          </UFormField>
            <UFormField label="1ère MEC">
            <PitInputDate v-model="vehicleForm.datePremiereMiseEnCirculation" />
          </UFormField>
          </div>
        </PitCard>
        ...
        <PitCard class="bg-white" v-else-if="step === 3">
          ...
          <div class="vo-form-grid">
            <UFormField label="Prix de vente souhaité">
            <PitInput v-model="depotForm.prixVenteSouhaite" />
          </UFormField>
            <UFormField label="Type de commission">
            <PitSelect v-model="depotForm.commissionType" class="vo-select">
                <option value="pourcentage">Pourcentage</option>
                <option value="fixe">Forfait</option>
              </PitSelect>
          </UFormField>
            <UFormField label="Valeur de commission">
            <PitInput v-model="depotForm.commissionValeur" />
          </UFormField>
            <UFormField label="Début mandat">
            <PitInputDate v-model="depotForm.dateDebut" />
          </UFormField>
            <UFormField label="Durée du mandat (jours)">
            <PitInput v-model="depotForm.dureeMandat" type="number" />
          </UFormField>
            <UFormField label="Gestionnaire VO">
            <PitSelect v-model="depotForm.gestionnaireId" class="vo-select">
                <option :value="null">Non assigné</option>
                <option v-for="user in experts" :key="user.id" :value="user.id">{{ user.prenom || user.username }} {{ user.nom || '' }}</option>
              </PitSelect>
          </UFormField>
            <UFormField label="Type pièce déposant">
            <PitSelect v-model="depotForm.deposantIdType" class="vo-select">
                <option value="carte_identite">Carte d'identité</option>
                <option value="passeport">Passeport</option>
                <option value="permis">Permis</option>
              </PitSelect>
          </UFormField>
            <UFormField label="N° pièce déposant">
            <PitInput v-model="depotForm.deposantIdNumber" />
          </UFormField>
            <UFormField label="Date pièce déposant">
            <PitInputDate v-model="depotForm.deposantIdDate" />
          </UFormField>
            <UFormField label="Conditions de restitution" class="vo-field-full">
            <PitTextarea v-model="depotForm.conditionsRestitution" :rows="3" />
          </UFormField>
            <UFormField label="Assurance / infos complémentaires" class="vo-field-full">
            <PitTextarea v-model="depotForm.assuranceInfo" :rows="3" />
          </UFormField>
            <UFormField label="Notes internes" class="vo-field-full">
            <PitTextarea v-model="depotForm.notes" :rows="3" />
          </UFormField>
          </div>
        </PitCard>
          ...
          <div class="vo-subsection">
            ...
            <div class="vo-lines">
              <div v-for="(row, index) in documentRows" :key="index" class="vo-doc-row">
                ...
                <PitInput type="file"  @change="onDocumentFileChange(index, $event)" />
        ...
        <div class="vo-footer-actions">
          ...
          <PitButton v-else type="button"  class="topbar-new-btn" color="primary">{{ submitting ? 'Création...' : 'Créer le dépôt' }}</PitButton>
        </div>
      </div>
...
definePageMeta({
  layout: 'default', title: 'Nouveau dépôt VO' })
