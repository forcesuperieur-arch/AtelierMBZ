<template>
  <div>
    <AppPageHeader title="Livre de Police" subtitle="Registre atelier des acquisitions et ventes VO.">
      <template #actions>
        <PitButton  class="topbar-new-btn" color="primary">
    ...
    <PitCard class="bg-white">
      <div class="vo-filters">
        <PitInput v-model="search" placeholder="N° ordre, immatriculation, vendeur, acheteur..." />
      </div>

      <PitTable :data="filteredEntries" :columns="columns">
...
definePageMeta({
  layout: 'default', title: 'Livre de Police VO' })
...
async function downloadPdf() {
  ...
  try {
    ...
    toast.add({ title: 'Erreur PDF', description: e instanceof Error ? e.message : 'Impossible de générer le PDF', color: 'error' })
  } finally {
