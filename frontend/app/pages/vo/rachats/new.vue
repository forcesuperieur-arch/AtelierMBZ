<template>
  <div>
    ...
    <div class="vo-companion-banner">
      ...
      <span>Le QR code du parcours VO apparaît directement. Tu peux soit remplir le dossier à la main, soit scanner les pièces vendeur et moto pour proposer les données du rachat avant validation interne.</span>
    </div>
    ...
    <div id="companion-qr-hero" class="vo-hero-qr">
      <div>
        ...
        <div class="vo-inline-actions" style="margin-top: 10px;">
          <PitButton type="button"  class="topbar-new-btn" color="primary">
          ...
          <PitButton v-if="draftPublicUrl" type="button"  class="vo-link-btn" color="primary">Copier le lien</PitButton>
        </div>
    ...
    <div class="vo-wizard-grid">
      <div>
        <PitCard class="bg-white" v-if="step === 1">
          <template #header>
            <div class="vo-card-title">1. Vendeur</div>
          </template>

          <div class="vo-block">
            <PitInput v-model="sellerSearch" label="Rechercher un client existant" placeholder="Nom, prénom, téléphone..." />
            ...
            <div class="vo-form-grid">
              <PitInput v-model="sellerForm.prenom" label="Prénom" />
              <PitInput v-model="sellerForm.nom" label="Nom" />
              <PitInput v-model="sellerForm.telephone" label="Téléphone" />
              <PitInput v-model="sellerForm.email" label="Email" />
              <div class="vo-field-full">
                <PitInput v-model="sellerForm.adresse" label="Adresse" />
              </div>
            </div>
          </div>
        </PitCard>

        <PitCard class="bg-white" v-else-if="step === 2">
          <template #header>
          ...
          <div class="vo-block">
            <div class="vo-inline-search">
              <div class="vo-field grow">
                <PitInput v-model="vehicleSearch" label="Recherche par immatriculation ou VIN" placeholder="Ex: AB-123-CD ou VIN" />
              </div>
              <PitButton  class="topbar-new-btn" color="primary">Rechercher</PitButton>
            </div>
            ...
            <div v-if="selectedVehicle" class="vo-selected-box">
              ...
              <PitButton type="button"  class="vo-link-btn" color="primary">Saisir un autre véhicule</PitButton>
            </div>

            <div class="vo-divider">ou créer rapidement</div>

            <div class="vo-form-grid">
              <PitInput v-model="vehicleForm.plaque" label="Immatriculation" />
              <div class="vo-field">
                <PitInput
                  v-model="vehicleForm.vin"
                  label="VIN"
                  :color="vehicleForm.vin && !isValidVin(vehicleForm.vin) ? 'error' : 'primary'"
                />
                <span v-if="vehicleForm.vin && !isValidVin(vehicleForm.vin)" class="vo-field-error">{{ vinErrorMessage(vehicleForm.vin) }}</span>
              </div>
              <div class="vo-field">
                <PitInput
                  v-model="vehicleForm.marque"
                  label="Marque"
                  @update:model-value="onVehicleMarqueInput"
                  @blur="hideVehicleMarqueSuggestions"
                />
              ...
              <div class="vo-field">
                <PitInput
                  v-model="vehicleForm.modele"
                  label="Modèle"
                  @update:model-value="onVehicleModeleInput"
                  @blur="hideVehicleModeleSuggestions"
                />
              ...
              <PitSelect
                v-model="vehicleForm.categorieId"
                label="Catégorie tarifaire"
                :options="[{ label: 'Non renseignée', value: '' }, ...categories.map(c => ({ label: c.nom, value: String(c.id) }))]"
              />
              <PitSelect
                v-model="vehicleForm.typeMoto"
                label="Type véhicule atelier"
                :options="[
              ...
              />
              <PitInput v-model="vehicleForm.cylindree" label="Cylindrée" placeholder="ex: 750" />
              <PitInput v-model="vehicleForm.annee" label="Année" type="number" />
              <PitInput v-model="vehicleForm.mileage" label="Kilométrage" type="number" />
              <PitInput v-model="vehicleForm.couleur" label="Couleur" />
              <PitInput v-model="vehicleForm.datePremiereMiseEnCirculation" label="1ère MEC" type="date" />
            </div>
          </div>
        </PitCard>

        <PitCard class="bg-white" v-else-if="step === 3">
          <template #header>
            <div class="vo-card-title">3. Financier</div>
          </template>

          <div class="vo-form-grid">
            <PitInput v-model="purchaseForm.purchasePrice" label="Prix d'achat" placeholder="0.00" />
            <PitInput v-model="purchaseForm.targetSalePrice" label="Prix de vente cible" placeholder="0.00" />
            <PitSelect
              v-model="purchaseForm.regimeTva"
              label="Régime TVA"
              :options="[
                { label: 'TVA sur marge', value: 'marge' },
                { label: 'TVA normale', value: 'normal' },
              ]"
            />
            <PitInput v-model="purchaseForm.purchaseDate" label="Date d'achat" type="date" />
            <PitSelect
              v-model="purchaseForm.sellerIdType"
              label="Type pièce vendeur"
              :options="[
            ...
            />
            <PitInput v-model="purchaseForm.sellerIdNumber" label="N° pièce vendeur" />
            <PitInput v-model="purchaseForm.sellerIdDate" label="Date pièce vendeur" type="date" />
            <PitInput v-model="purchaseForm.nonGageDate" label="Date non-gage" type="date" />
          </div>

          <PitCheckbox v-model="purchaseForm.controleTechniqueOk" checkbox-label="Contrôle technique vérifié" />

          <div class="vo-subsection">
            <div class="vo-subtitle-row">
              <strong>Frais de remise en état</strong>
              <PitButton type="button"  class="vo-link-btn" color="primary">+ Ajouter une ligne</PitButton>
            </div>
            <div class="vo-lines">
              <div v-for="(item, index) in freItems" :key="index" class="vo-line-row">
                <PitInput v-model="item.label" placeholder="Libellé" />
                <PitInput v-model="item.amount" placeholder="0.00" />
                <PitButton type="button"  class="vo-link-btn" color="primary">Supprimer</PitButton>
              </div>
            </div>
          </div>

          <div class="vo-field-full">
            <PitTextarea v-model="purchaseForm.notes" label="Notes internes" :rows="4" />
          </div>
        </PitCard>

        <PitCard class="bg-white" v-else>
          <template #header>
          ...
          <div class="vo-form-grid">
            <PitSelect
              v-model="purchaseForm.expertId"
              label="Expert / référent VO"
              :options="[{ label: 'Non assigné', value: null }, ...experts.map(u => ({ label: `${u.prenom || u.username} ${u.nom || ''}`, value: u.id }))]"
            />
          </div>
          ...
          <div class="vo-subsection">
            <div class="vo-subtitle-row">
              ...
              <PitButton type="button"  class="vo-link-btn" color="primary">+ Ajouter un document</PitButton>
            </div>
            <div class="vo-lines">
              <div v-for="(row, index) in documentRows" :key="index" class="vo-doc-row">
                <PitSelect
                  v-model="row.type"
                  :options="[{ label: 'Type', value: '' }, ...purchaseDocumentOptions]"
                />
                <PitInputDate :model-value="row.dateExpiration" @update:model-value="v => row.dateExpiration = v" />
                <PitFileInput :model-value="[]" @update:model-value="onDocumentFileChange(index, $event)" />
                <PitButton type="button"  class="vo-link-btn" color="primary">Supprimer</PitButton>
              </div>
            </div>
          </div>
          ...
          <PitCheckbox
            v-model="purchaseForm.createAndConfirm"
            checkbox-label="Créer puis confirmer immédiatement"
            :disabled="missingConfirmationDocs.length > 0"
          />
        ...
        <div class="vo-footer-actions">
          ...
          <PitButton v-if="step < 4" type="button"  class="topbar-new-btn" color="primary">Continuer</PitButton>
          <PitButton v-else type="button"  class="topbar-new-btn" color="primary">{{ submitting ? 'Création...' : 'Créer le dossier' }}</PitButton>
        </div>
      </div>

      <div>
        <PitCard class="vo-summary-panel bg-white">
          <template #header>
...
definePageMeta({
  layout: 'default', title: 'Nouveau rachat VO' })
...
const documentRows = ref<{ type: string; dateExpiration: string; file: File | null }[]>([{ type: '', dateExpiration: '', file: null }])
...
function onDocumentFileChange(index: number, files: File[]) {
  documentRows.value[index]!.file = files[0] ?? null
}
